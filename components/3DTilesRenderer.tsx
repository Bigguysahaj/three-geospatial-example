...
import { EffectComposer } from '../lib/helpers/EffectComposer'
import { HaldLUT } from '../lib/helpers/HaldLUT'
import { googleMapsApiKeyAtom, needsApiKeyAtom } from '../lib/helpers/states'
import { Stats } from '../lib/helpers/Stats'
import { useColorGradingControls } from '../lib/helpers/useColorGradingControls'
import { useControls } from '../lib/helpers/useControls'
import { useGoogleMapsAPIKeyControls } from '../lib/helpers/useGoogleMapsAPIKeyControls'
import { useKeyboardControl } from '../lib/helpers/useKeyboardControl'
import {
  useLocalDateControls,
  type LocalDateControlsParams
} from '../lib/helpers/useLocalDateControls'
import { usePovControls } from '../lib/helpers/usePovControls'
import { useToneMappingControls } from '../lib/helpers/useToneMappingControls'
import { useCloudsControls } from '../lib/clouds/helpers/useCloudsControls'

const CITY_PRESETS: Record<string, SceneProps> = {
  tokyo: {
    dayOfYear: 170,
    timeOfDay: 7.5,
    exposure: 10,
    ...
    <TilesRenderer
      ...
      {apiKey !== '' ? (
        ...
        <TilesPlugin
          plugin={GoogleCloudAuthPlugin}
          args={{
        apiToken: process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY,
        autoRefreshToken: true
          }}
        />
      )}
...
interface GeospatialMapProps extends Partial<SceneProps> {
  city?: keyof typeof CITY_PRESETS | string;
}

export const GeospatialMap: FC<GeospatialMapProps> = ({ city = 'tokyo', ...customProps }) => {
  useGoogleMapsAPIKeyControls()
  const needsApiKey = useAtomValue(needsApiKeyAtom)

  const sceneProps = useMemo(() => {
    const cityKey = city.toLowerCase() as keyof typeof CITY_PRESETS;
    const preset = CITY_PRESETS[cityKey] || CITY_PRESETS.tokyo;
  ...
  return (
    <>
      ...
      {needsApiKey && (
        <div className="absolute top-1/2 left-1/2 text-white text-center leading-normal -translate-x-1/2 -translate-y-1/2">
